...
int RBFTest_p5(PagedFileManager *pfm) {
	...
	rc = fileHandle.collectCounterValues(readPageCount, writePageCount,
			appendPageCount);
	if (rc != success) {
		cout << "[FAIL] collectCounterValues() failed. Test Case p5 failed."
				<< endl;
	...
	rc = fileHandle.collectCounterValues(readPageCount1, writePageCount1,
			appendPageCount1);
	if (rc != success) {
		cout << "[FAIL] collectCounterValues() failed. Test Case 13 failed."
				<< endl;
		rc = pfm->closeFile(fileHandle);
		return -1;
	}
	assert(
			readPageCount1 - readPageCount == 1
					&& "Read counter should be correct.");
	assert(
			writePageCount1 - writePageCount == 1
					&& "Write counter should be correct.");
	assert(
			appendPageCount1 - appendPageCount == 2
					&& "Append counter should be correct.");
	assert(
			appendPageCount1 > appendPageCount
					&& "The appendPageCount should have been increased.");

	// Get the number of pages
	unsigned count = fileHandle.getNumberOfPages();
	assert(count == (unsigned )2 && "The count should be one at this moment.");

	// Close the file "testp5"
	rc = pfm->closeFile(fileHandle);
	...
	cout << readPageCount << " " << readPageCount1 << endl;
	assert(
			readPageCount1 - readPageCount == 1
					&& "Persistent read counter should be correct.");
	assert(
			writePageCount1 - writePageCount == 1
					&& "Persistent write counter should be correct.");
	assert(
			appendPageCount1 - appendPageCount == 2
					&& "Persistent append counter should be correct.");
	...
	cout << "RBF Test Case Private 5 Finished! The result will be examined."
			<< endl << endl;
