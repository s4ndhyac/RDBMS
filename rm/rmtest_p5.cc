...
int TEST_RM_PRIVATE_5(const string &tableName)
{
    ...
    if(rc != success) {
        free(returnedData);
	    cout << "***** RM Test Case Private 5 failed. *****" << endl << endl;
        return -1;
    }
    ...
    while(rmsi2.getNextTuple(rid, returnedData) != RM_EOF)
    {
        ...
        {
            cout << "***** A wrong entry was returned. RM Test Case Private 5 failed *****" << endl << endl;
             rmsi2.close();
             free(returnedData);
             free(value);
             return -1;
    ...
    if (counter != 97){
        cout << "***** The number of returned tuple: " << counter << " is not correct. RM Test Case Private 5 failed *****" << endl << endl;
     } else {
