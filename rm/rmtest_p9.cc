...
RC TEST_RM_PRIVATE_9(const string &tableName)
{
    ...
    if (rc == success) {
		cout << "***** [FAIL] The system catalog should not be altered by a user's insertion call. RM Test Case Private 9 failed. *****" << endl;
		free(returnedData);
		free(buffer);
		return -1;
    }
	...
	if (rc == success){
		cout << "***** [FAIL] The system catalog should not be deleted by a user call. RM Test Case Private 9 failed. *****" << endl;
		free(returnedData);
		free(buffer);
		return -1;
	}
    ...
    if (attrs[1].name == "table-name") {
       projected_attrs.push_back(attrs[1].name);
		...
		cout << "***** [FAIL] The system catalog implementation is not correct. RM Test Case Private 9 failed. *****" << endl;
		free(returnedData);
		free(buffer);
		return -1;
    }
