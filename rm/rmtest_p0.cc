...
int TEST_RM_PRIVATE_0(const string &tableName)
{
    ...
    if(rc != success) {
	    ...
	    cout << "***** RM Test Case Private 0 failed. *****" << endl << endl;
        return -1;
    }
    int counter = 0;
    int ageReturned = 0;
    ...
    while(rmsi.getNextTuple(rid, returnedData) != RM_EOF)
    {
       ...
       if (ageReturned % 2 != 0){
             cout << "***** A returned value is not correct. RM Test Case Private 0 failed. *****" << endl << endl;
             rmsi.close();
             free(returnedData);
             for(int i = 0; i < numTuples; i++)
             {
    ...
    if (counter != numTuples / 2){
         cout << "***** The number of returned tuples:" << counter << ". RM Test Case Private 0 failed. *****" << endl << endl;
         free(returnedData);
         for(int i = 0; i < numTuples; i++)
         {
             free(tuples[i]);
    ...
    if(rc != success) {
        cout << "***** RelationManager::deleteTale() should not fail. RM Test Case Private 0 failed. *****" << endl << endl;
		free(returnedData);
		for(int i = 0; i < numTuples; i++)
		{
			free(tuples[i]);
