...
RC TEST_RM_PRIVATE_1(const string &tableName)
{
    ...
    for(int i = 0; i < numTuples; i++)
    {
        ...
        string msg = "Txt" + ss.str();
        string referred_topics = "Rft" + ss.str();
	...
	for(int i = 0; i < numTuples; i=i+10)
    {
        ...
        stringstream ss;
        ss << setw(5) << setfill('0') << i;
        string msgToCheck = "Txt" + ss.str();
        string referred_topicsToCheck = "Rft" + ss.str();
        ...
        if (testFail) {
		    cout << "***** RM Test Case Private 1 failed on " << i << "th tuple - attr: " << attrID << "*****" << endl << endl;
		    free(returnedData);
