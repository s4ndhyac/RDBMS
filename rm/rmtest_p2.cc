...
RC TEST_RM_PRIVATE_2(const string &tableName)
{
	...
	nullsIndicatorWithNulls[0] = 92; // 01011100 - the 2nd, 4th, 5th, and 6th column is null. - userid, send_time, referred_topics, and message_text
    ...
    for(int i = 0; i < numTuples; i++)
    {
		...
		if (i%50 == 0) {
				...
				if (!nullBit) {
					cout << "A returned value from a readAttribute() is not correct: attrID - " << attrID << endl;
					testFail = true;					
				}		
		} else {
			int value = 0;
        ...
        if (testFail) {
		    cout << "***** RM Test Case Private 2 failed on the tuple #" << i << " - attrID: " << attrID << "*****" << endl << endl;
		    free(returnedData);
